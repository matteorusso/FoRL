...
class Rollout(object):
    def __init__(
        ...
        self.reward_fun = (
            lambda ext_rew, int_rew: ext_rew_coeff
            * np.clip(ext_rew, -1.0, 1.0)
        ...
        self.recorder = (
            Recorder(nenvs=self.nenvs, nlumps=self.nlumps)
            if record_rollouts
            else None
        )
    ...
    def calculate_reward(self):
        if self.policy.is_NSI:
            ...
            self.buf_rews[:] = self.reward_fun(
                int_rew=int_rew, ext_rew=self.buf_ext_rews
            )
    ...
    def rollout_step(self):
        ...
        if s == self.nsteps_per_seg - 1:
            for l in range(self.nlumps):
                sli = slice(l * self.lump_stride, (l + 1) * self.lump_stride)
                nextobs, ext_rews, nextnews, _ = self.envs[l].step_async(
                    self.acs_record[l]
                )
                ...
                if t == self.nsteps - 1:
                    ...
                    (
                        _,
                        self.buf_vpred_last[sli],
                        _,
                    ) = self.policy.get_ac_value_nlp(nextobs)
    ...
    def update_info(self):
        ...
        if all_ep_infos:
            all_ep_infos = [
                i_[1] for i_ in all_ep_infos
            ]  # remove the step_count
        ...
        if current_max is not None:
            if (self.best_ext_ret is None) or (
                current_max > self.best_ext_ret
            ):
