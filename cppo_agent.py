...
class NSIOptimizer(object):
    ...
    def start_interaction(self, env_fns, nlump=2):
        self.optimizer_NSN = torch.optim.Adam(
            self.stochpol.param_list_NSN, lr=self.lr
        )
        self.optimizer_NSN.zero_grad()

        self.optimizer_IDN = torch.optim.Adam(
            self.stochpol.param_list_IDN, lr=self.lr
        )
        self.optimizer_IDN.zero_grad()

        self.optimizer_VFN = torch.optim.Adam(
            self.stochpol.param_list_VFN, lr=self.lr
        )
        ...
        self.envs = [
            VecEnv(
                env_fns[l * self.lump_stride : (l + 1) * self.lump_stride],
    ...
    def update(self):
        ...
        if self.normrew:
            ...
            self.rff_rms.update_from_moments(
                rffs_mean, rffs_std ** 2, rffs_count
            )
        ...
        for _ in range(self.nepochs):
            ...
            for start in range(
                0, self.nenvs * self.nsegs_per_env, envsperbatch
            ):
...
class PpoOptimizer(object):
    ...
    def start_interaction(self, env_fns, dynamics, nlump=2):
        param_list = (
            ...
            + self.dynamics.param_list
            + self.dynamics.auxiliary_task.param_list
        )  # copy a link, not deepcopy.
    ...
    def update(self):
        ...
        for _ in range(self.nepochs):
            ...
            for start in range(
                ...
                feat_var = torch.std(
                    self.dynamics.auxiliary_task.features.float()
                )

                total_loss = (
                    pg_loss + ent_loss + vf_loss + feat_loss + dyn_loss
                )
                ...
                to_report["pg"] += pg_loss.data.numpy() / (
                    self.nminibatches * self.nepochs
                )
                to_report["vf"] += vf_loss.data.numpy() / (
                    self.nminibatches * self.nepochs
                )
